...
if(isset($_COOKIE['session'])  && is_string($_COOKIE['session']) && !empty($_COOKIE['session']) && isset($_SESSION['session']) && !empty($_SESSION['session']) ){
    if($_COOKIE['session'] === $_SESSION['session']){
        ...
        if( $Department == 'PRE' || $Department == 'V_PRE' ){
        ...
        JOIN 
            users ON tasks.id = users.id
        JOIN 
            personal_info ON users.id = personal_info.id
        WHERE 
            personal_info.Did = '$Department'
        ORDER BY 
            tasks.date DESC;";
...
<body style="background-color: #3B3B3B; color: white;">
    <div class="w3-sidebar w3-bar-block w3-collapse w3-card w3-animate-left side_nav" style="background: rgb(37,37,37);background: linear-gradient(rgba(37,37,37,1) 0%, rgba(42,42,42,1) 45%, rgba(14,14,14,1) 100%);" id="mySidebar">
        ...
        <div class="navlinks">
            <ul>
                ...
                <li><a href="#" class="navlink2">My Tasks</a></li>
    ...
    <?php 
        ...
        $stmt->bind_result($role);
        $stmt->fetch();
        if(($role == 'admin') && $Department != 'PRE' && $Department != 'V_PRE'){
            echo "<div class='creat text-end'><a href='create_tasks.php'>+New Task</a></div>";
            $creat = true;
        }
    ?>
    <?php
    ...
    <?php
        if($no){
            echo "<h1 class='no text-center'>No Tasks yet.</h1>";
        }
    ?>
