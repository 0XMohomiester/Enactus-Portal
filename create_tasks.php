...
// check if the user's browser store the cookie or not.
if(isset($_COOKIE['session']) && is_string($_COOKIE['session']) && !empty($_COOKIE['session'])  && isset($_SESSION['session']) && !empty($_SESSION['session'])){
    ...
    if($_COOKIE['session'] === $_SESSION['session']){
        ...
        $stmt->bind_param("i", $id_create_task);
        ...
        if(!($role === 'admin' && ($Did != 'PRE' || $Did != 'V_PRE'))){
        ...
        if(isset($_POST['submit'])){
            if(isset($_POST['title']) && isset($_POST['content']) && !empty($_POST['title']) && !empty($_POST['content'])){
                if(is_string($_POST['title']) && is_string($_POST['content'])){
                    ...
                    $create_task_sql = "INSERT INTO `tasks` (`id`, `Did`,`Title`, `Content`, `date`) VALUES (?, ?, ?, ?, ?)";
                    $stmt = $conn->prepare($create_task_sql);
                    $stmt->bind_param("issss", $id_create_task, $Did, $title, $content, $date);
                    $stmt->execute();  // send to the database
                    $stmt->close();
...
<div class="back">
    <a href="tasks.php"><i class="bi bi-arrow-left-short"></i></i></a>
</div>
<script>
    /////////////////////////////////////////
